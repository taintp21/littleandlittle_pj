...
<h1>
    {{ trans('event::events.title.create event') }}
</h1>
<ol class="breadcrumb">
    <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
    <li><a href="{{ route('admin.event.event.index') }}">{{ trans('event::events.title.events') }}</a></li>
    <li class="active">{{ trans('event::events.title.create event') }}</li>
</ol>
@stop

@section('content')
{!! Form::open(['route' => ['admin.event.event.store'], 'method' => 'post']) !!}
<div class="row">
    <div class="col-md-10">
            ...
            <div class="tab-content">
                ...
                @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                    ...
                    <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                        @include('event::admin.events.partials.create-fields', ['lang' => $locale])
                    </div>
                @endforeach
